...
callLaterFunction typ time var value =
  Call (ID $ "later_" ++ typeName typ) [ var, time, value ]
...
callAssignFunction typ lvalue priority rvalue =
  Call (ID $ "assign_" ++ typeName typ) [ lvalue, priority, rvalue ]
...
callSensitizeFunction v t = Call (ID "sensitize") [ Cast (Pointer sv_t) v
                                                  , Pre Address t]

-- | Expression for calling "desensitize" on a trigger.  Argument is
-- the trigger.
callDesensitizeFunction :: Expression -> Expression
callDesensitizeFunction t = Call (ID "desensitize") [Pre Address t]


-- | Type of a "bare" activation record
bare_act_t :: CType
bare_act_t = namedType "act_t"

-- | C type of a trigger
trigger_t :: CType
trigger_t = namedType "trigger_t"

-- | C type of a priority number
priority_t :: CType
priority_t = namedType "priority_t"

-- | C type of a priority "depth" number
depth_t :: CType
depth_t = namedType "depth_t"
...
cgen modName (I.Program fs) = TranslationUnit $
                      ...
                      TopVerbatim "#include \"peng-platform.h\"" :
                      concatMap toFunc fs
