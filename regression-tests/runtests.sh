...
PENG="stack exec peng-exe --"

# For when we are in the "regression-tests/out" directory
export PENGLIB=../../runtime

CC="cc -O -Wall -pedantic -std=c99 -I ../runtime/include -I ../runtime/linux/include -I . -I out"
LINK="cc -L out"
...
Check() {
    ...
    if [ ! -f "out/libpeng.a" ] ; then
	Run mkdir -p out &&
	Run make -C out -f $PENGLIB/linux/Platform.mk libpeng.a "1>&2"
    fi
    ...
    Run $PENG "--module-name=${basename}" "--generate-h" "$1" ">" "${cheader}" &&
    Run $PENG "--module-name=${basename}" "--generate-c" "$1" ">" "${csource}" &&
    Run $CC -c -o "${obj}" "${csource}" &&
    if [ -f "${mainsource}" ] ; then
	NoteGen "${mainobj} ${exec} ${result} ${diff}"
	Run $CC -c -o "${mainobj}" "${mainsource}" &&
	Run $LINK -o "${exec}" "${obj}" "${mainobj}" -lpeng &&
	Run "${exec}" ">" "${result}" &&
...
CheckFail() {
    ...
    RunFail $PENG "$1" ">" "${csource}" "2>" "${errfile}" &&
